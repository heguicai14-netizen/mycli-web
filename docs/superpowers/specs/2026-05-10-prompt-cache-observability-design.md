...
| OpenRouter | 同上(两条路径都尝试) | OpenRouter 透传上游 usage;默认 parser 先匹配 OpenAI 路径,失败后回落到 DeepSeek 路径,因此 OpenRouter 路由到任一上游都能命中 |
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| GLM 偶发不返回 cached(冷缓存) | live test flake | live test 不 assert `cached > 0`,改为:第二轮 done.usage 必须存在,且 `cached` 字段类型为 `number` 或 `undefined`(只验证链路打通,不验证命中) |
...
未来若要加 Anthropic 原生 adapter(改 CLAUDE.md 约束后),`NormalizedUsage` + `UsageParser` 的形状能容纳 Anthropic 的 `usage.cache_read_input_tokens`(对应 `cached`)。Anthropic 还有 `cache_creation_input_tokens`(写缓存的 token 数)是独立维度,届时可以 additive 在 `NormalizedUsage` 上加 `cacheCreated?: number`,本 spec 不预先引入以免过度设计。
